...
namespace App\Exports\FinanceDivision;
...
class EscrowExport implements FromView
{
    ...
    {
        $transaction = Transaction::select('uuid')->where('is_active', 1)->where('category', 'escrow')->where('status', 4)->where(function($query){
        ...
        foreach($tempTrans as $tr){
            $model = Transaction::where('is_active', 1)->where('category', 'escrow')->where('uuid', $tr)->where('status', 4)->with(['transactionAccount', 'transactionProject'])->orderBy('updated_at', 'desc')->get();
            $transUuid = [];
        ...
        if($this->request->format == 'excel'){
            return view('finance-division.escrow.excel', ['dataTrans' => $dataTrans]);
        }
        elseif($this->request->format == 'csv'){
            return view('finance-division.escrow.csv', ['arrayData' => $arrayData]);
        }
    }
}
