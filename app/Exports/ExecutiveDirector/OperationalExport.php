...
class OperationalExport implements FromView
{
    ...
    {
        $transaction = Transaction::select('uuid')->where('is_active', 1)->where('type', 2)->where('category', 'operational')->where('status', 4)->where(function($query){
        ...
        if($this->request->format == 'excel'){
            return view('executive-director.operational.excel', ['dataTrans' => $dataTrans]);
        }
        elseif($this->request->format == 'csv'){
            return view('executive-director.operational.csv', ['arrayData' => $arrayData]);
        }
    }
}
