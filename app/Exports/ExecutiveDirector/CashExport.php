...
class CashExport implements FromView
{
    ...
    {
        $transaction = Transaction::select('uuid')->where('type', 2)->where('is_active', 1)->where('category', 'cash')->where('status', 4)->where(function($query){
        ...
        if($this->request->format == 'excel'){
            return view('executive-director.cash.excel', ['dataTrans' => $dataTrans]);
        }
        elseif($this->request->format == 'csv'){
            return view('executive-director.cash.csv', ['arrayData' => $arrayData]);
        }
    }
}
