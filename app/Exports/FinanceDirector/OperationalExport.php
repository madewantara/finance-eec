...
class OperationalExport implements FromView
{
    ...
    {
        $transaction = Transaction::select('uuid')->where('is_active', 1)->where('type', 2)->where('category', 'operational')->where(function($query){
        ...
        foreach($tempTrans as $tr){
            $model = Transaction::where('is_active', 1)->where('type', 2)->where('category', 'operational')->where('uuid', $tr)->where(function($query){
                $query->where('status', 3)
                ->orWhere('status', 4);
        ...
        if($this->request->format == 'excel'){
            return view('finance-director.operational.excel', ['dataTrans' => $dataTrans]);
        }
        elseif($this->request->format == 'csv'){
            return view('finance-director.operational.csv', ['arrayData' => $arrayData]);
        }
    }
}
